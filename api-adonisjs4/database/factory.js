...
const Hash = use('Hash') // Para poder cifrar los password

// La factoría para rellenar con datos falsos la tabla USERS. Primer parámetro se le pasa el modelo y el segundo será de forma asíncrona.
// faker nos proporcionará los datos falsos
Factory.blueprint('App/Models/User', async (faker) => {
  return {
    username: faker.username(),
    email: faker.email(),
    password: await Hash.make(faker.password()), // Cifrar password
...
// i viene de índice. El parámetro DATA son datos que van a venir desde fuera. Es otra forma de hacerlo
Factory.blueprint('App/Models/Room', async (faker, i, data) => {
  const rows = faker.integer({ min: 5, max: 20 });
  return {
    cinema_id: data.cinema_id,
    room_rows: rows,
    room_seats: rows * 10,// número de sitios por cada fila
...
Factory.blueprint('App/Models/MovieShowing', async (faker, i, data) => {
  return {
    ...
    movie_show_date: new Date() // Fecha actual
  }
})
...
Factory.blueprint('App/Models/Booking', async (faker, i, data) => {
  return {
    ...
    booking_made_date: new Date(), // Fecha actual
