...
const actions = {
    [types.actions.fetchMovies]: ({commit}, cinemaId) => {
        commit(globalTypes.mutations.startProcessing); // Empezar una petición. Bloqueo con blockUI
        Vue.http.get(`movies/${cinemaId}/byCinema`).then(movies => {
            commit(types.mutations.receivedMovies, {apiResponse: movies});
            commit(globalTypes.mutations.stopProcessing); // Se para el proceso para que se oculte blockUI
        })
    },
...
const getters = {
    [types.getters.movies]: (state) => {
        let movies = state.cinemaInfo.movie_showings; // Esto son las películas que se encuentran en los cines
        if(state.query.search) {
            // Como 'movies' tiene la info de movie_showings, si vemos las relaciones en el servidor se ve que movie_showings tiene relación con la tabla movie. Por eso
            // se puede acceder a 'movie_name' de la tabla movie. Una vez accedido se puede hacer la búsqueda o nos quedamos con las fila que tenga lo mismo que (state.query.search)
        ...
        if(state.query.genre) {
            ...
            // Nos devolverá las películas al seleccionar un cierto genero
...
const mutations = { // El parámetro state está declarado más arriba en esta página
