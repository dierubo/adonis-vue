    ...
    <div class="BookingSeats__Wrapper">
        <table class="table">
            <tr v-for="row in room.room_rows">
                <td align="center" v-for="seat in seatsPerRow" v-bind:class="{booked: seatsIds.includes(currentSeat(row, seat)), selected: pending_booked.indexOf(`${row}-${currentSeat(row, seat)}`) > -1}" click="selectSeat(row, currentSeat(row, seat), seat)">
        ...
        <div v-if="pending_booked.length">
            <hr />
            <button class="btn btn-warning btn-block" @click="processBooked">{{ $t('booking.process_book') }}</button>
            <hr />
        </div>
    </div>
</template>
