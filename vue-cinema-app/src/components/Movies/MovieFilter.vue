<template>
    <div class="MovieFilter__Wrapper">
        <div class="well">
            ...
            <button class="btn btn-block btn-warning" @click="clearFilter">{{ $t('filter.clear') }}</button>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...mapActions({
            fetchGenres: movieTypes.actions.fetchGenres // Para obtener todos los géneros y mostrarlos en un dropdown
        }),
        ...mapMutations({ // Mutaciones que se van a ejecutar cuando se estén cambiando los valores del dropdown
    ...
    computed: {
        hours () { // Desplegable con las horas
            let hours = [], i = 0;
            for(i; i < 24; i++) {
                let hour = i < 10 ? `0${i}` : i;
                hours.push({
        ...
        hour: {
            ...
            set (value) {
                this.setHour(value); // Esto accede a la mutación dentro de MapMutations setHour
            }
        },
        search: {
            get () {
